...
interface SignUp {
    SignUp(params: { email: String, first_name: String, last_name: String, password: String }): Promise<any>;
}
            ...
            if (target[p] == null) {
                ...
                switch (p) {
                    ...
                    case 'Login':
                        ...
                        target[p] = async function ({ email, password }: Parameters<auth['Login']>[0]) {

                            return new Promise((resolve, reject) => {
                                userSchema.findOne({ email }).then(async (user: any) => {
                                    if (!user) reject(user);
                                    try {
                    ...
                    case 'SignUp':
                        ...
                        target[p] = async function ({ email, first_name, last_name, password }: Parameters<auth['SignUp']>[0]) {
                            return new Promise(async (resolve, reject) => {
                                try {


                                    const encryted_password = await bcrypt.hash(password.toString(), saltRounds)
                                    console.log(encryted_password);
                    ...
                    case 'details': 
                        ...
                        target[p] = async function ({ _id }: Parameters<auth['details']>[0]) {


                            return await userSchema.find({ _id }).exec()
